...
if (!empty($_FILES['files']['tmp_name'])) {
    foreach ($_FILES['files']['tmp_name'] as $key => $tmpName) {
        ...
        if (move_uploaded_file($tmpName, $fullPath)) {
            ...
            mysqli_query($conn, "
                INSERT INTO surat_masuk_files (surat_id, nama_file, file_path)
...
if (!empty($uploaded)) {
    $cek = mysqli_query($conn, "SELECT id FROM dokumen WHERE sumber='surat_masuk' AND sumber_id='" . (int)$suratId . "'");
